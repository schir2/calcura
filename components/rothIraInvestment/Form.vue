<template>
  <n-card role="dialog" class="max-w-2xl" :bordered="true">
    <template #header>
      <h3 class="text-2xl">RothIraInvestment: {{ rothIraInvestmentPartial.name }}</h3>
    </template>
...
interface Props {
  rothIraInvestmentPartial: Partial<RothIraInvestment>;
  mode: 'create' | 'edit'
}
...
const formFields = ref(useFieldHelpers(rothIraInvestmentForm, defineField))
