...
select isnt_empty('select * from income', 'user_a sees own income');

-- 2. other user cannot see owner's row
set local role authenticated;
select tests.jwt_authenticated('00000000-0000-0000-0000-000000000002');
select is_empty('select * from income', 'user_b cannot see user_a income');
...
select lives_ok(
    $$delete from income$$,
    'user_b delete on income is silent'
);

-- 6. owner's row survives
set local role authenticated;
select tests.jwt_authenticated('00000000-0000-0000-0000-000000000001');
select isnt_empty('select * from income', 'user_a income row survives user_b delete attempt');
