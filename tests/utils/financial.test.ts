...
describe('financialUtils', () => {
    ...
    describe('getAnnualAmount', () => {
        ...
        it('should calculate annual amount for Annually frequency', () => {
            expect(getAnnualAmount(500, 'annual')).toBe(500);
        });

        it('should calculate annual amount for Weekly frequency', () => {
            expect(getAnnualAmount(50, 'weekly')).toBe(2600);
        });

        it('should calculate annual amount for Quarterly frequency', () => {
            expect(getAnnualAmount(300, 'quarterly')).toBe(1200);
        });

        it('should return 0 for amount of 0, regardless of frequency', () => {
            expect(getAnnualAmount(0, 'monthly')).toBe(0);
            expect(getAnnualAmount(0, 'annual')).toBe(0);
            expect(getAnnualAmount(0, 'weekly')).toBe(0);
            expect(getAnnualAmount(0, 'quarterly')).toBe(0);
        });
        ...
        it('should throw an error for invalid frequency', () => {
            expect(() => getAnnualAmount(100, 'InvalidFrequency' as any)).toThrow();
        });
    });
