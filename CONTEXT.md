...
**Status: restored inside the Entity Workspace.** The legacy `<domain>/TemplatePicker.vue` was deleted by [ADR 012](docs/adr/012-retire-legacy-modal-form-stack.md); the tables and `processTemplate()` were **kept**. Template selection now lives in the [[Entity Workspace]]: `openCreate(model, planId, seed?)` carries optional seed values, the drawer threads them into the domain's `WorkspaceForm` as `initialValues` (create mode only), and each domain's create-mode form exposes a "Start from a template" picker that merges the chosen `*_template` row over the form defaults via `processTemplate()`. Wired end-to-end for **debt** first; other domains follow the same pattern.
