...
const STRATEGY_OPTIONS = [
  ...
  {value: 'percentage_of_income', label: '% of Total Income', hint: 'A share of your total household income'},
...
function handleSubmit() {
  onSubmit(async () => {
    const {id: _id, ...rest} = model.value as Brokerage
    if (id === null) {
      await store.create({...rest, plan_id: planId} as BrokerageInsert)
      if (planId !== null) await commandSequenceStore.fetchByPlan(planId)
    } else {
      await store.patch(id, rest as BrokerageUpdate)
    }
    emit('saved')
  })
}
...
<template>
  ...
  <n-form v-else ref="formRef" :model="model" :rules="rules">
    <n-form-item label="Name" path="name" :show-feedback="false" class="!mb-3">
      <n-input ref="nameInput" v-model:value="model.name" placeholder="Enter investment name"/>
    </n-form-item>
    ...
    <common-strategy-rows v-model="model.contribution_strategy" :options="STRATEGY_OPTIONS">
      ...
      <template #percentage_of_income>
        <n-form-item label="Percentage of income" path="contribution_percentage" :show-feedback="false">
