...
const STRATEGY_OPTIONS = [
  ...
  {value: 'percentage_of_income', label: '% of Income', hint: 'A share of an associated income'},
...
onMounted(async () => {
  if (!incomeStore.list.length) incomeStore.fetchAll()
  if (id !== null) {
...
function handleSubmit() {
  onSubmit(async () => {
    const {id: _id, ...rest} = model.value as Ira
    if (id === null) {
      await store.create({...rest, plan_id: planId} as IraInsert)
      if (planId !== null) await commandSequenceStore.fetchByPlan(planId)
    } else {
      await store.patch(id, rest as IraUpdate)
    }
    emit('saved')
  })
}
...
<template>
  ...
  <n-form v-else ref="formRef" :model="model" :rules="rules">
    <n-form-item label="Name" path="name" :show-feedback="false" class="!mb-3">
      <n-input ref="nameInput" v-model:value="model.name" placeholder="Enter IRA name"/>
    </n-form-item>
    ...
    <common-strategy-rows v-model="model.contribution_strategy" :options="STRATEGY_OPTIONS">
      <template #fixed>
        <n-form-item label="Amount / yr" path="contribution_fixed_amount" :show-feedback="false">
          <base-number-slider v-model="model.contribution_fixed_amount" :min="0" :max="8000" :step="500"/>
        </n-form-item>
      </template>
