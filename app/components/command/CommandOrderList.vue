...
// One lockable, draggable ordering list — used twice by CommandSequence (contributions + withdrawals).
const props = defineProps<{
  commands: CommandSequenceCommandWithRelations[]
...
const emit = defineEmits<{
  ...
  'toggle-lock': []
  'toggle-command': [csc: CommandSequenceCommandWithRelations]
  update: [payload: { modelName: ModelName, id: number, data: Record<string, unknown> }]
  delete: [payload: { modelName: ModelName, id: number }]
}>()

// Local copy for vuedraggable; resynced whenever the parent's ordered subset changes.
const localCommands = ref<CommandSequenceCommandWithRelations[]>([...props.commands])
watch(() => props.commands, (next) => {
  localCommands.value = [...next]
}, {deep: true})
...
function onEnd() {
  drag.value = false
  emit('reorder', localCommands.value.map(csc => csc.id))
}

// Collapse/expand: per-item overrides seed from the global toggle; dragging collapses everything and
// restores per-item state on drop. Precedence: drag override → per-item state ← global toggle.
...
<template>
  <div>
    ...
    >
      <n-button
          size="tiny"
          :type="locked ? 'info' : 'default'"
          :secondary="locked"
          @click="emit('toggle-lock')"
      >
        <template #icon>
          <base-ico :name="locked ? 'lock' : 'unlock'"/>
        </template>
        {{ locked ? 'Locked — predefined' : 'Unlocked — custom' }}
      </n-button>
      <span :class="locked ? 'text-blue-600 dark:text-blue-300' : 'text-skin-muted'">
        {{ locked ? predefinedHint : 'drag handles to reorder' }}
      </span>
      <span class="flex-1"/>
    ...
    <draggable class="dragArea list-group w-full space-y-1"
      ...
      <template #item="{element: command} : {element: CommandSequenceCommandWithRelations}">
        <div class="flex gap-2 items-center border-skin-base/30 bg-skin-surface rounded border p-1 transition"
          ...
          <base-ico
              :class="['text-2xl drag-handle', locked ? 'opacity-20 cursor-not-allowed' : 'text-skin-primary/80 cursor-move']"
              name="drag"
          />
          <n-switch
              :round="false"
              ...
              @update:value="() => emit('toggle-command', command)"
          />
          <component
              ...
              @update="(id: number, update: Record<string, unknown>) => emit('update', {modelName: command.command.model_name, id, data: update})"
              @delete="(id: number) => emit('delete', {modelName: command.command.model_name, id})"
          />
        </div>
      </template>
    </draggable>
