...
const model = ref<Partial<Income>>({})
...
onMounted(async () => {
  try {
    const fetchedData = await store.fetch(props.id)
...
function handleSubmit() {
  onSubmit(async () => {
    const { id: _id, ...update } = model.value as Income
    emit('update', props.id, update as IncomeUpdate)
  })
}
</script>
<template>
  ...
  <n-card v-else role="dialog" :class="FORM_MODAL_WIDTH_CLASS" :bordered="true">
    ...
    <template #default>
      ...
      <Bar v-if="data" id="income-update-chart"
