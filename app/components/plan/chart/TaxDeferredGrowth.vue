<template>
  <n-card>
    <template #header>
      <p v-if="finalPlanState" class="text-xl text-center">Tax Deferred Savings at Age {{ finalPlanState.plan.age }}</p>
    </template>
...
const series = computed(() => props.states.map(state => state.assets.tax_deferred.balance_end))
